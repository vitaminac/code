...
public class MatrixMultiplicationInt {
	...
	public MatrixMultiplicationInt() {
		MatrixMultiplicationInt.LEAF_SIZE = 1;
	}

	public MatrixMultiplicationInt(int n) {
		MatrixMultiplicationInt.LEAF_SIZE = n;
	}

	public static int[][] iterativeMatrixMult(int[][] A, int[][] B) {
		int n = A.length;
	...
	private static int[][] strassenR(int[][] A, int[][] B) {
		...
		if (n <= LEAF_SIZE) {
			return iterativeMatrixMult(A, B);
		} else {
